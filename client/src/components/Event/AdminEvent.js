...
function AdminEvent() {
    ...
    const comments = (id) => {
        navigate("/Comments", { state: { eventId: id } })
    ...
    const approve = (id) => {
        ...
        axios.post("http://localhost:5000/api/approveEvent", data)
            .then(function (response) {
                console.log(response);
                getdata()
                toast.success('Approved Removed', {
    ...
    const remove = (id) => {
        ...
        axios.post("http://localhost:5000/api/removeEvent", data)
            .then(function (response) {
                console.log(response);
                getdata()
                    ...
                    <Table sx={{ minWidth: 650 }} aria-label="simple table" color="#F7F6F2">
                        ...
                        <TableHead sx={{ color: 'white' }}>
                            <TableRow >
                                ...
                                <TableCell className={styles.headTitle} >Approved</TableCell>
                        ...
                        <TableBody >
                            ...
                            (
                                <TableRow key={item.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                    ...
                                    <TableCell className={styles.ind} component="th">{item.approved === 0 ? "NO" : "YES"}</TableCell>
                                    <TableCell component="th" align='center'>
                                        <Stack direction="row" spacing={0} sx={{ marginTop: '-15px' }}>
                                            ...
                                            {item.approved === 0 && <IconButton color="primary" aria-label="add to shopping cart"><DoneOutlineIcon style={{ color: '#009EFF' }} onClick={() => approve(item.id)} /></IconButton>}
                                            ...
                                            <Button sx={{ height: "30px", marginTop: '6px' }} variant="contained" onClick={() => comments(item.id)}>COMMENTS</Button>
                                        </Stack>
                                    </TableCell>
                                </TableRow>
                            ))}
